...
class ErrorWindow(tk.Toplevel):
    def __init__(self, master, title, text, telemetry):
        ...
        sub = ttk.Label(container, text=sub_text, font=("Segoe UI", 10))
        sub.pack(anchor="w", pady=(2, 8))

        if sub_sub_text:
            sub_sub = ttk.Label(
                container, text=sub_sub_text, font=("Segoe UI", 10), foreground="red"
            )
        ...
        ttk.Button(btn_frame, text="Копировать логи", command=self.copy_text).pack(
            side="left"
        )
        ...
        email_link.bind(
            "<Button-1>",
            lambda e: open_email_client(
                "romat3422@gmail.com", "Error log", error_log_email_text
            ),
        )
        ...
        telegram_link.bind(
            "<Button-1>", lambda e: webbrowser.open("https://t.me/br0mand")
        )

        ttk.Button(btn_frame, text="Закрыть", command=self.close).pack(side="right")

...
from arcade import draw_lbwh_rectangle_filled, uicolor, draw_lrbt_rectangle_filled, draw_circle_filled, draw_circle_outline
from arcade.gui import Surface, UISliderStyle, UIBaseSlider
...
class UISliderSavesUpdater(agui.UISlider):
    ...
    def on_event(self, event: agui.UIEvent) -> bool | None:
        super().on_event(event)

        if agui.UIMouseMovementEvent is type(event):
            if self in [
        ...
        if agui.UIMouseReleaseEvent is type(event) and self.directed:
            match self.type:
                case "music":
...
class UISliderVertical(agui.style.UIStyledWidget[UISliderStyle], UIBaseSlider):
    ...
    @override
    def on_event(self, event: agui.UIEvent) -> bool:
        """Handle UI events for the slider."""
        if isinstance(event, (agui.UIMouseMovementEvent, agui.UIMouseDragEvent)):
        ...
        elif isinstance(event, agui.UIMousePressEvent):
        ...
        elif isinstance(event, agui.UIMouseReleaseEvent):
            if self.pressed:
                self.pressed = False
        ...
        elif isinstance(event, agui.UIOnUpdateEvent):
            # Обновляем состояние наведения (только если не нажато)
